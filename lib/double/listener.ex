defmodule Double.Listener do
  ...
  def record({_mod, _function_name, _args} = mfa, delegated_from) do
    GenServer.call(__MODULE__, {:record, mfa, delegated_from})
  end
  ...
  def handle_call({:record, mfa, {pid, _tag} = _delegated_from}, _from, state) do
    state = [{pid, mfa} | state]
    {:reply, state, state}
  end
